        ...
        implements ConnectorSplitSource
{
    ...
    public static final String RESULT_FILES_QUERY = "SELECT rtrim(path) FROM stl_unload_log WHERE query=pg_last_query_id() ORDER BY path";
    ...
    public CompletableFuture<ConnectorSplitBatch> getNextBatch(int maxSize)
    {
        ...
        List<ConnectorSplit> splits = getPaths().stream()
                .map(path -> new JdbcSplit(Optional.of(path)))
