...
, diffs AS (
    SELECT
      ...
      , 100 * (run_devs.mean - lag(run_devs.mean) OVER w) / nullif(cast(greatest(run_devs.mean, lag(run_devs.mean) OVER w) as real), 0) AS diff_pct
      -- details
      , run_devs.mean
      , run_devs.stddev
...
SELECT
  ...
  , nullif(format('[%s](runs/%s.md)', props_num, id), '[](runs/.md)') AS "Run number"
  , env_name AS "Environment"
  , metric AS "Metric"
  --, unit AS "Unit"
  , format_metric(diff, unit) AS "Diff"
  , cast(diff_pct AS decimal(5,2)) AS "Diff %"
  , format_metric(mean, unit) AS "Mean"
  , format_metric(stddev, unit) AS "stddev"
  , cast(stddev_pct AS decimal(5,2)) AS "stddev %"
  , '[' || format_metric(min, unit) || ', ' || format_metric(max, unit) || ']' AS "Range"
  , run_properties AS "Run properties"
FROM diffs
ORDER BY group_id, is_header DESC, query_name, run_properties, env_name
;
