...
SELECT
    left_env_name AS "Left environment"
  , right_env_name AS "Right environment"
  --, run_vars
  , query_name AS "Query name"
  , metric AS "Metric"
  , metric_scope AS "Metric scope"
  --, unit AS "Unit"
  , format_metric(diff, unit) AS "Diff"
  , cast(diff_pct AS decimal(5,2)) AS "Diff %"
  , format_metric(left_mean, unit) AS "Left mean"
  , format_metric(left_stddev, unit) AS "Left stddev"
  , cast(left_stddev_pct AS decimal(5,2)) AS "Left stddev %"
  , '[' || format_metric(left_min, unit) || ', ' || format_metric(left_max, unit) || ']' AS "Left range"
  , format_metric(right_mean, unit) AS "Right mean"
  , format_metric(right_stddev, unit) AS "Right stddev"
  , cast(right_stddev_pct AS decimal(5,2)) AS "Right stddev %"
  , '[' || format_metric(right_min, unit) || ', ' || format_metric(right_max, unit) || ']' AS "Right range"
  , format('[%1$s](runs/%1$s.md)', left_run_id) AS "Left run ID"
  , format('[%1$s](runs/%1$s.md)', right_run_id) AS "Right run ID"
FROM diffs_ranked
WHERE rownum < 6
